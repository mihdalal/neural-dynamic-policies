...
save_dir = '/home/sbahl2/research/dmp_rl/data/benchmark/zip/'  + str('{:05d}'.format(args.expID))
T = args.T
algo = 'demos'
num_epochs = args.num_epochs
os.makedirs(save_dir, exist_ok=True)
ims = np.load('/home/sbahl2/research/dmp_rl/data/benchmark/new_pour/processed_ims.npy', allow_pickle=True).item()[algo]
trajs = np.load('/home/sbahl2/research/dmp_rl/data/benchmark/new_pour/processed_trajs.npy', allow_pickle=True).item()[algo]
...
if len(X_train.shape) > 4:
    X_train, X_test, Y_train, Y_test = np.concatenate(X_train, axis=0), np.concatenate(X_test, axis=0), np.concatenate(Y_train, axis=0), np.concatenate(Y_test, axis=0)
...
state_index = np.arange(7)
vel_index = None
dmpn = DMPNet(N=N, l=l, T=T, a_z=a_z, hidden_size=hidden_sizes,state_index=state_index, vel_index=vel_index, freeze=args.freeze)
...
for epoch in range(num_epochs):
    ...
    if epoch % 100 == 0:
        ...
        print('Epoch: ' + str(epoch) + ', train_loss: ' + str(l) + ', test_loss: '  + str(test_loss.item()))
